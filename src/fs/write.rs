use fuser::{Request, ReplyWrite};
use crate::{get, FS, FILE_SIZE, get_mut, EDIT_TIMES, CHANNEL_ID, WEBHOOK};
...
pub fn write(req: &Request<'_>, ino: u64, _fh: u64, mut offset: i64, data: &[u8], _write_flags: u32, _flags: i32, _lock_owner: Option<u64>, reply: ReplyWrite) {
                ...
                match y {
                    Item::File(x) => {
                        ...
                        if offset >= 0 && offset <= x.size as i64 {
                            ...
                            let mut part = reqwest::blocking::get(format!("https://cdn.discordapp.com/attachments/{}/{}/discord-fs", get!(CHANNEL_ID), get_attachment(*new_files.last().unwrap()).unwrap())).unwrap().bytes().unwrap()[..offset_location as usize].to_vec();
                            ...
                            while data.len()-taken_data > 0 {
                                if data.len()-taken_data > FILE_SIZE as usize {
                                    data_chunks.push(data[taken_data..taken_data+FILE_SIZE as usize].to_vec());
