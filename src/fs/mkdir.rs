use crate::{FS, get, get_mut, TTL, webhook};
use fuser::{Request, ReplyEntry};
use std::ffi::OsStr;
use crate::controller::{Item, Directory, Attr};
use std::time::SystemTime;
use crate::fs::access::check_access;
use libc::EACCES;
use crate::fs::create::next_inode;

pub fn mkdir(req: &Request<'_>, parent: u64, name: &OsStr, mode: u32, _umask: u32, reply: ReplyEntry) {
    ...
    if check_access(attr.uid, attr.gid, attr.permissions, req.uid(), req.gid(), 0b010) {
        ...
        get_mut!(FS).insert(inode, Item::Directory(Directory {
            files: vec![],
            attr: Attr {
                ino: inode,
